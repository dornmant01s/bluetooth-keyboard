...
class ChannelLaunchService : AccessibilityService() {
    ...
    fun isPlus(ev: KeyEvent): Boolean {
        return ev.keyCode == KeyEvent.KEYCODE_NUMPAD_ADD ||
               (ev.keyCode == KeyEvent.KEYCODE_EQUALS && ev.isShiftPressed)
    }
    fun isMinus(ev: KeyEvent): Boolean {
        return ev.keyCode == KeyEvent.KEYCODE_NUMPAD_SUBTRACT ||
               ev.keyCode == KeyEvent.KEYCODE_MINUS
    }
